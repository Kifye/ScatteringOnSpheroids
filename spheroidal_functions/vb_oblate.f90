      ...
      subroutine coblfcn(c,m,lnum,ioprad,x,r1c,ir1e,r1dc,ir1de,r2c,&
                   ...
                   is1e,s1dc,is1de, enr, maxd)
!
!  subroutine version of the fortran program coblfcn
!
!      version 1.03
!      September 2020
!
...
!               respect to the shape parameter x for a given value of
...
!               s1 and their first derivatives with respect to the angle
!               coordinate eta for a given value of c and m and for a
!               specified number of degrees l = m, m+1,..., m+lnum-1
!
!  This program for complex c is a major advance in capability
!  from coblfcn version 1.01 and an improvement over version 1.02.
!  It is written in fortran 90. It was developed around oblfcn
!  version 1.24 which obtains spheroidal function values for real
!  values of the size parameter c = kd/2, where k is the wavenumber
!  and d is the interfoocal distance of the elliptical cross-section
!  of the oblate spheroid. A description of the methods used in
...
!  factors.
!
!  A description of the methods used in coblfcn as well as a brief
!  description of this program is provided in the article 'Calculation
!  of oblate spheroidal wave functions with complex argument,' available
!  at arXiv.org, identifier 2009.01618, August 2020.
!
!  Coblfcn can be run in either double precision [64 bit; real(8) and
!  complex(8)], quadruple precision [128 bit; real(16) and complex(16)]
!  arithmetic, or a hybrid where everything except the Bouwkamp
!  procedure to refine the eigenvalue is run in double precision
!  arithmetic. In the latter case, coblfcn switches to quadruple
!  precision for the Bouwkamp procedure in those cases where double
!  precision fails to provide highly accurate eigenvalues. See the
!  discussion below about when to choose which arithmetic. The choice
!  of arithmetic is set in the module param located at the end of
!  coblfcn. Here, the kind parameters knd and knd1 are set by the
!  statements:
...
!  arithmetic or to 16 for 128 bit arithmetic. knd controls the
!  arithmetic for all but the Bouwkamp procedure
!  Set the value of knd1 to be used for the Bouwkamp procedure.
!  Note that if knd = 16, knd1 should also = 16.
!
...
!  statement 5 below the comments section to the number of bytes for
!  double precision data and quadruple precision data, respectively.
!  Increasing the values of kindd and kindq will improve the accuracy
!  and can significantly extend the paramteter ranges over which
!  coblfcn provides useful results.
!
!  Following is a description of input and output. Included is a
!  discussion of the output file fort.60 that alerts the user when the
!  accuracy of the radial and/or angular functions fall below a
!  specified minimum value and when the eigenvalue routine fails. Next
!  is a discussion of the ranges of parameters for which coblfcn
!  provides useful results. Following that is a discussion of how to
!  obtain the expansion d coefficients and the various angular function
!  normalizations. Finally is a discussion of four optional output files
!  fort.20, fort.30, fort.40, and fort.50 that may be of interest.
!
!     Input and Output
!
...
!                   interfocal length) (either complex*16 or complex*32)
!
...
!                   = 0) (either real*8 or real*16)
!
!          r1c   :  either complex*16 or complex*32 vectors of length
!          r1dc     lnum containing the characteristics for the radial
!                   functions of the first kind r1 and their
!                   first derivatives
!
!          ir1e   : integer vectors of length lnum containing the
!          ir1de    exponents corresponding to r1c and r1dc
!
!          r2c    : complex*16 or complex*32 vectors of length lnum
!          r2dc     containing the characteristics for the radial
!                   functions of the second kind r2 and their first
!                   derivatives
!
...
!                   values of l (complex*16 or complex*32)
...
!     Output file fort.60
!
!  Coblfcn provides a file fort.60 that should be of interest to the
!  user, especially when using coblfcn outside the ranges where useful
!  results are expected. See the discussion of these ranges below.
!  Whenever the estimated accuracy of the radial functions falls below a
!  designated integer value, the associated values of x, c, m, and l are
!  written to fort.60. The integer is currently set equal to 6 in write
!  statements found after the line numbered 1405 below in subroutine
!  main. I expect that accuracies as low as 5 or 6 digits and possibly
!  even as low as 4 digits may be sufficient in many cases, especially
!  when they occur very rarely. The integer can be set higher if desired.
!
!  Coblfcn will also write to fort.60 whenever the estimated accuracy
!  of the Meixner and Schafke normalization for the angular functions
!  is less than the same integer used for alert about tne radial
!  function accuracy. It provides the values of m, l, and c where this
!  occurs together with the value ndec - jsubms -1, where ndec is the
!  precision and jsubms is the subtraction error in calculating the
!  normalization.
!
!  Fort.60 also alerts the user to the unlikely situation where the
!  eigenvalue procedure has failed for one or more values of l and the
!  same eigenvalue is obtained for two different values of l (of the
!  same parity). In this case one or more eigenvalues are missing and
!  the results are not usable. I do not expect this to be a possibility
!  unless coblfcn is used with ci and cr beyond the ranges stated below
!  for which useful results are expected.
!
!  If the user does not desire fort.60, it can be avoided by searching
!  for write(60 and placing a c in column 1 to commenting out the write
!  statement that are found. The open statement can also be commented
!  out.
!
!     Useful parameter ranges for coblfcn
!
...
!  precision if available. Note that this increases the run time by a
...
!  below for the accuracy integer naccr in the section for fort.20.
...
!  of x ranging from 0.000001 to 10 as well as the special case of
!  x = 0, values for cr up to 5000, and values of ci up to 200. Testing
!  for both the double precision and hybrid versions included all values
!  of the order m from 0 to 200 and from 210 to 1000 in steps of 10.
!  Testing for the quadruple precision version included values of m from
!  0 to 200 in steps of 10 and from 250 to 1000 in steps of 50. For all
!  versions the values of the degree l ranged from m to m + lnum -1,
!  where lnum was chosen sufficiently large that the magnitudes of r1
!  and r1d were less than 10**(-300).
!
...
!  results for small x. At least 5 digits are obtained for cr up to 5000
!  for all of the values of x down to 0.000001.
!
...
!
!  When ci = 20, there are 5 or more digits of accuracy for all tested
!  values of x when cr <= 100. There are 5 or more digits of accuracy
!  at x >= 0.1 for cr = 150 and at x >= 0.2 for cr up to 2000. Testing
...
!  than it does for 128 bit arithmetic. Running coblfcn on an ordinary
!  laptop computer can take a long time using 128 bit arithmetic when
!  cr is very large.
!
!  Testing for values of ci up to 40 showed that coblfcn provides useful
!  results for cr up to at least 5000 and for m up to 1000 and x down to
!  0.000001. Estimated accuracies for the radial functions were 8 or
!  more digits except for a possible rare 6 or 7 digit result. See the
!  discussions of estimated accuracies given below for the special case
!  of x = 0 and for the angular functions.
!
!  Testing for ci = 50 shows useful results for cr up to at least 2000.
!  Accuracies for the radial functions were almost always at least 8
!  more digits but occasional accuracies as low as 5 digits were seen
!  near the breakpoint, primarily for cr >= 1000 and m > 200. When cr
!  = 2000, there were even a few 4 digit results for m >= 700 and a few
!  3 digit results for m >= 800. It is unlikely that values of m this
!  large will be required by the user.
!
!  Testing for ci = 60 shows useful results for cr <= 100 although a
!  few 4 digit results occurred at x = 0.00001 and 0.000001. Results
!  for cr = 200 were similar except that there were a few more 4 digit
!  results at x = 0.00001 and 0.00001 and some 4 digit results now at
!  x = 0.0001 and x = 0.001. Testing for cr = 500 shows that useful
!  results are only obtained for x >= 0.05 for all m and for x < 0.05
!  for m up to about 160. Testing for cr = 1000 and 2000 showed useful
!  results for x >= 0.2 for all m with a possible rare 4 digit result.
!
!  Testing for ci = 70 shows useful results for cr <= 100 when x >=
!  0.01, for cr = 150 when x >= 0.05, for cr = 200 when x >= 0.1, for
!  cr = 500 when x >= 0.3 and for cr = 1000 when x >= 0.5.
!
!  Testing for ci = 80 shows useful results for cr <= 20 when x >=
!  0.01, for cr = 50 when x >= 0.02, for cr = 100 when x >= 0.2,
!  for cr = 200 when x >= 0.3, for ci = 300 when x >= 0.5, and for
!  ci = 400 when x >= 1.1.
!
!  Testing for yet higher values of ci showed a continued increase in
...
!
!  Output data can be written to the following four files: fort.20,
!  fort.30, fort.40, fort.50. These are currently suppressed. If
!  desired, one or more of these files, say fort.40, can be obtained
!  by searching for write(40 statements and removing the c given in
!  column 1 of each of them as well as their associated format
!  statements. Also remove the c from the open(40 statement near the
!  beginning of the program. Note that the open statements allow
!  default values to apply for the specs. These can, of course, be
!  changed.
!
!   fort.20
!
...
!     in the radial functions as measured using the Wronskian (except
!     for (1) the case where x is equal to zero, (2) the case where
!     the Wronskian is used to obtain a leading coefficient for the
!     Legendre function expansion method and (3) the case where both
!     the radial functions of the first kind R1 and those of the
!     second kind R2 are very large such that r2 and r2d are very nearly
!     equal to -i times r1 and -i times r1d, respectively. [see comments
!     below regarding naccr]
!
...
!                        larger than those allowed in 64 bit
!                        arithmetic on the users computer since
!                        the floating point function values
...
!                        integer naccr. When ci is small, r1 and r1d
!                        are very accurate so that naccr usually
!                        relects the accuracy of r2 and r2d.
!
...
!                        the estimated accuracy of r1 and r1d. See the
...
!     a desired value of the angular coordinate eta followed by the
!     corresponding angular functions and accuracy. Specific output
!     and format for each line is as follows.
!
!        for iopang = 1:
!
!               barg   ; angular coordinate eta
...
!                        less accurate the larger the value of c, the
...
!   fort.40 and fort.50
!
!     These files are diagnostic files that contain information about
!     specific techniques used and numbers of terms required for the
!     radial function and angular function calculations, respectively.
!     They are annotated and should be self explanatory.
!
!
!        use param
!
...
!  complex scalars and arrays
        complex(knd) c,r1c(lnum),r1dc(lnum),r2c(lnum),&
               r2dc(lnum),s1c(lnum,narg),s1dc(lnum,narg)
!
!  integer arrays
...
!  and to 8 decimal digits when aimag(c) > 20.
!  These can be changed if desired. See comments
!  below about changing minacc
!
...
10      maxp=max(maxn,maxp,maxpdr)
     ...
     60.0e0_knd.and.real(c).ge.10.0e0_knd.and.mmin.le.40&
     .and.x.le.0.99e0_knd.and.x.gt.0.1e0_knd)&
...
!
        ...
        call main (mmin,minc,mnum,lnum,c,ioprad,iopang,iopnorm,minacc,&
             ...
             jnebmax,ndec,nex,xneu,r1c,ir1e,r1dc,ir1de,r2c,ir2e,&
             r2dc,ir2de,s1c,is1e,s1dc,is1de,kindd,kindq,&
				enr(:, 1:maxd))
!
        end
!
!
        subroutine main (mmin,minc,mnum,lnum,cc,ioprad,iopang,iopnorm,&
                   ...
                   maxq,maxt,neta,jnenmax,jnebmax,ndec,nex,xneu,&
                   r1c,ir1e,r1dc,ir1de,r2c,ir2e,r2dc,ir2de,s1,&
                   is1,s1d,is1d,kindd,kindq, enr)
!
...
!  real(knd) and complex(knd) scalars
        real(knd) aj1,aj2,apcoef,apcoefn,api,arg1,c,coefn,coefn1,&
            coefme,coefmo,coefme1,coefmo1,darg,dconp,dec,deta,&
            em,etaval,factor,factor1,pcoef,pcoefn,pi,qdm0,&
            qdm1,qm0,qm1,rm,rm2,step1,step2,step3,ten,term,&
            termpq,teste,testeo,t1,t2,t3,t4,t5,t6,t7,wm,x,xb,&
            xbninp,xl,xhigh,xlow,xneu
        real(knd) ang,apcoef1,etaval1,pcoefe,pcoefet,pcoefo,&
            pdcoefe,pdcoefo,pdcoefet,pcoefe1,pcoefet1,pcoefo1,&
            pdcoefe1,pdcoefo1,pdcoefet1
...
!
        complex(knd) cc,c2,c4,dfnorm,dmlf,dmfnorm,dmlmf,dmsnorm,dmlms,&
               ...
               r1dcm,r1d1c,r1ec,r1dec,r1dcest,r2sumest,r2ic,r2dic,&
               r2lc,r2dlc,r2l1c,r2dl1c,r2nc,r2dnc,r2ec,r2dec,&
               wronc,wronca,wroncb,wront
        complex(knd) c3
        complex(knd1) c21,c41
!
!  integer arrays with dimension lnum
        integer   ir1e(lnum),ir1de(lnum),ir2e(lnum),ir2de(lnum),&
            match(lnum + 1),iqdl(lnum),iql(lnum),ieigt(lnum),&
            is1(lnum,narg),is1d(lnum,narg)
!
...
!  complex(knd) arrays with dimension maxd
		complex(knd) enr(lnum, maxd)
        complex(knd), allocatable, dimension(:) :: bliste,&
	blisto,gliste,glisto,drhor,pint1,pint2,pint3,&
  pint4,rpint1,rpint2,sbesf,sbesdf,sbesfe,&
  sbesdfe,sbesdr,sbesn,sneudr1,sneudr2,sneudre,sbesne,&
  sbesdre,enrneg,sneufe,sneudfe,sneuf1,sneudf1,&
  sneuf2,sneudf2,sneun1,sneun2,sneune,s1c,s1dc,eigt,&
	f,g,d,e

		complex(knd), allocatable, dimension(:,:) :: sbesfsv,&
  sbesdfsv,sneudrsv,sbesnsv,sbesdrsv,sneufsv,&
  sneudfsv,sneunsv

        complex(knd1), allocatable, dimension(:) :: bliste1,&
	blisto1,gliste1,glisto1
!
...
!  integer and real(knd) arrays with dimension maxlp
        integer, allocatable, dimension(:) :: ibese,ipnormint,&
	ipnormint1,ibesee,ineue1,ineue2,ineuee,ipdnorm,ipdnorma,&
	ipnorm,ipnorma,ipdtempe,ipdtempo,iptempe,iptempo,is1e,is1de,&
  naccs,naccds,neeb,neeb1,limpsv,limp1sv,limnsv,jelimsv,&
	jelim1sv,limjsv


		integer, allocatable, dimension(:,:) :: ibesesv,ineuesv

        real(knd), allocatable, dimension(:) :: pnormint,pnormint1,&
	qdqr,prat1,alpha,beta,coefa,coefb,coefc,coefd,coefe,gamma,&
  pdratt,pratb,pratt,pratb1,pratt1,pdratt1,prx,pdrx,&
	qrat,qdrat,qrat1,qdrat1,arg,etainp,pdnorm,&
  pdnorma,pnorm,pnorma,pdtempe,pdtempo,ptempe,ptempo,xin,&
  xlninp,wr,xr,eta,xbn,xln,wmeta2

		real(knd) :: barg(maxt)


		real(knd), allocatable, dimension(:,:) :: pdr,pdrat,&
  pr,prat,pratbsv,prattsv,pdrattsv,pratbsv1,prattsv1,&
  pdrattsv1
!
...
!
! ALLOCATION
		allocate(bliste(maxd),blisto(maxd))
		allocate(gliste(maxd),glisto(maxd))
		allocate(drhor(maxdr))
		allocate(pint1(maxint),pint2(maxint),pint3(maxint))
		allocate(pint4(maxint),rpint1(maxint),rpint2(maxint))
		allocate(sbesf(maxj),sbesdf(maxj),sbesfe(maxj))
		allocate(sbesdfe(maxj),sbesfsv(jnebmax,maxj))
		allocate(sbesdfsv(jnebmax,maxj))
		allocate(ibese(maxlp),ipnormint(maxlp),ipnormint1(maxlp))
		allocate(ibesee(maxlp))
		allocate(ibesesv(jnebmax,maxlp))
		allocate(pnormint(maxlp),pnormint1(maxlp))
		allocate(sbesdr(maxlp),sbesn(maxlp),sneudr1(maxlp))
		allocate(sneudr2(maxlp),sneudre(maxlp))
		allocate(sneudrsv(jnenmax,maxlp),sbesne(maxlp))
		allocate(sbesdre(maxlp),sbesnsv(jnebmax,maxlp))
		allocate(sbesdrsv(jnebmax,maxlp))
		allocate(qdqr(maxmp))
		allocate(enrneg(maxmp))
		allocate(prat1(maxn))
		allocate(ineue1(maxn),ineue2(maxn),ineuee(maxn))
		allocate(ineuesv(jnenmax,maxn))
		allocate(sneufe(maxn),sneudfe(maxn),sneufsv(jnenmax,maxn))
		allocate(sneudfsv(jnenmax,maxn),sneuf1(maxn),sneudf1(maxn))
		allocate(sneuf2(maxn),sneudf2(maxn),sneun1(maxn))
		allocate(sneun2(maxn),sneune(maxn),sneunsv(jnenmax,maxn))
		allocate(alpha(maxp),beta(maxp),coefa(maxp),coefb(maxp))
		allocate(coefc(maxp),coefd(maxp),coefe(maxp),gamma(maxp))
		allocate(pdr(maxt,maxp),pdrat(maxt,maxp),pdratt(maxp))
		allocate(pr(maxt,maxp),prat(maxt,maxp),pratb(maxp))
		allocate(pratbsv(jnenmax,maxp),prattsv(jnenmax,maxp))
		allocate(pdrattsv(jnenmax,maxp),pratt(maxp))
		allocate(pratb1(maxp),pratt1(maxp),pdratt1(maxp))
		allocate(pratbsv1(jnebmax,maxp),prattsv1(jnebmax,maxp))
		allocate(pdrattsv1(jnebmax,maxp))
		allocate(prx(maxpdr),pdrx(maxpdr))
		allocate(qrat(maxq),qdrat(maxq),qrat1(maxq),qdrat1(maxq))
		allocate(arg(maxt),etainp(maxt),pdnorm(maxt))
		allocate(pdnorma(maxt),pnorm(maxt),pnorma(maxt),pdtempe(maxt))
		allocate(pdtempo(maxt),ptempe(maxt),ptempo(maxt),xin(maxt))
		allocate(xlninp(maxt))
		allocate(ipdnorm(maxt),ipdnorma(maxt),ipnorm(maxt))
		allocate(ipnorma(maxt),ipdtempe(maxt),ipdtempo(maxt))
		allocate(iptempe(maxt),iptempo(maxt),is1e(maxt),is1de(maxt))
		allocate(naccs(maxt),naccds(maxt))
		allocate(s1c(maxt),s1dc(maxt))
		allocate(wr(ngau),xr(ngau))
		allocate(eigt(maxe2))
		allocate(f(maxe),g(maxe),d(maxe),e(maxe))
		allocate(eta(neta),xbn(neta),xln(neta),wmeta2(neta))
		allocate(neeb(jnenmax),neeb1(jnebmax),limpsv(jnenmax))
		allocate(limp1sv(jnebmax),limnsv(jnenmax),jelimsv(jnenmax))
		allocate(jelim1sv(jnebmax),limjsv(jnebmax))
...
!280           format(1x,'l = ',i6)
            ...
            jint.gt.jbes) limdrad=jint+jint+20+int(sqrt(c))
              ...
              if((iopneu0.ne.0.and.li.ge.listart).or.(x.ge.xneu&
               .and.li.eq.listart)) limd=max(limd,lneu0)
                if(knd.eq.kindd) then
            ...
            leta=jeta+jeta+40+&
              ...
              if((iopeta.ne.0.and.li.ge.listart).or.(x.ge.0.05e0_knd&
               .and.li.eq.listart)) limd=max(limd,leta)
290           continue
...
350           continue
              ...
              if(ix.eq.0) call conver (l,m,lnum,cc,limd,bliste,gliste,&
                                 bliste1,gliste1,ndec,maxd,ioprad,&
                                 minacc,eigest,eigmat,lipl,lips,&
                                 liplp,mat,eigp,eign,kindd,kindq,&
                                 eigval,enr(li,:),ienre,itestme,&
                                 naccre,ieigt,iopeige,iflage,&
								   kflag)
              if(ix.eq.1) call conver (l,m,lnum,cc,limd,blisto,glisto,&
                                 blisto1,glisto1,ndec,maxd,ioprad,&
                                 minacc,eigest,eigmat,lipl,lips,&
                                 liplp,mat,eigp,eign,kindd,kindq,&
                                 eigval,enr(li,:),ienro,itestmo,&
...
380           call dnorm (l,m,cc,ndec,nex,limd,maxd,enr(li,:),ioprad,&
                    iopang,dc01,idc01,dfnorm,idfe,dmlf,idmlfe,&
                    dmfnorm,idmfe,dmlmf,idmlmfe,dmsnorm,idmse,&
                    dmlms,idmlmse,jnorm,jsubf,jsubmf,jsubms)
        ...
        iopleg.eq.0.and.iopleg1.eq.0.and.l.ge.legstart) iopleg=1
          ...
          .and.iopneu0.eq.0.and.(iopleg.eq.0.or.nacclegp.lt.&
          minacc).and.(iopint.eq.0.or.naccintp.lt.minacc).and.&
          (iopleg1.eq.0.or.naccleg1p.lt.minacc).and.iopeta.eq.0)&
               iopneu0=4
              if(istartneu0.eq.0) iopneu0=0
...
!     1               'using nonzero term in traditional Bessel ',
!     2               'function expansion')
!              if(knd.eq.kindd) write(40,570) r1c(li),ir1e(li),r1dc(li),
!     1                                   ir1de(li)
!              if(knd.eq.kindq) write(40,575) r1c(li),ir1e(li),r1dc(li),
!     1                                   ir1de(li)
              if(ioprad.eq.1) go to 1330
!              if(ix.eq.0) write(40,500)
!500           format(5x,'calculated r2 using nonzero term in Legendre',
!     1              ' function expansion and r2d from Wronskian and r1')
!              if(ix.eq.1) write(40,510)
!510           format(5x,'calculated r2d using nonzero term in ',
!     1               'Legendre function expansion and r2 from ',
!     2               'Wronskian and r1d')
!              if(knd.eq.kindd) write(40,520) r2c(li),ir2e(li),r2dc(li),
!     1                                   ir2de(li)
...
!560           format(4x,'r1 and r1d calculation')
              ...
              call r1bes(l,m,cc,x,limr1,ndec,maxd,enr(li,:),maxj,maxn,&
                   maxlp,nex,iflag,sbesf,sbesdf,sbesn,ibese,&
                   sbesdr,prat1,pcoefn,ipcoefn,dmfnorm,idmfe,&
                   ir1ep,r11c,ir11e,r1d1c,ir1d1e,jbes1,nsub,&
					 nsubd)
              jbes=jbes1
              iopbes=2
...
!     1                f35.31,f35.31,i5)
           ...
           .or.(li.eq.lips+1.and.liplp.ne.lipl+1)).and.l.ne.m&
          .and.max(nsub,nsubd,jsubmf).gt.nsubt) then
                nee1=1
                idir=0
                iflageta1=0
                jflageta1=0
...
670               ibesesv(jnencur1,je)=ibesee(je)
              ...
              call pleg(m,limpd,maxp,limcsav,iopd,ndec,etainp,netainp,&
                  maxt,prat,pdrat,pdnorma,ipdnorma,pnorma,&
...
710           continue
              ...
              call r1eta(l,m,cc,x,etaval1,nee1,limeta,ndec,nex,maxd,&
                   maxlp,maxj,maxp,minacc,wm,enr(li,:),sbesfe,&
                   sbesne,ibesee,sbesdfe,sbesdre,pdratt1,pratb1,&
                   pratt1,pcoefet1,ipcoefet1,pdcoefet1,ipdcoefet1,&
                   ir1ep,r1ec,ir1ee,r1dec,ir1dee,nsub1,nsubd1,&
					 jeta1)
              if(iopbes.eq.0) jbes=jeta1
              if(iopbes.ne.0) jbes=max(jbes,jeta1)
720           continue
!              if(knd.eq.kindd) write(40,730) etaval1,nee1,r1ec,ir1ee,
!     1                                       r1dec,ir1dee
!              if(knd.eq.kindq) write(40,735) etaval1,nee1,r1ec,ir1ee,
!     1                                     r1dec,ir1dee
!730           format(15x,'eta = ',f12.9,'; nee = ',i4,/,10x,'r1 = ',
!     1                f19.15,f19.15,i5,5x,'r1d = ',f19.15,f19.15,i5)
!735           format(15x,'eta = ',f12.9,'; nee = ',i4,/,10x,'r1 = ',
!     1                f35.31,f35.31,i5,/,5x,'r1d = ',f35.31,f35.31,i5)
               ...
               jsubmf+1.and.li.gt.lipl.and.l.gt.nbp) then
                  iopbes=1
                  go to 750
                  end if
              ...
              nsubd1.gt.nsubd1m+1).and.min(nsub1m,nsubd1m).lt.&
              nsub1mt).or.(nsub1.gt.nsub1m+3.and.nsubd1.gt.&
              nsubd1m+3)) then
                    r1c(li)=r1cm
                    ir1e(li)=ir1em
                    r1dc(li)=r1dcm
               ...
               nsubd1.le.nsubd1m+1).and.min(nsub1m,nsubd1m)&
               .lt.nsub1mt)) then
                    r1c(li)=r1cm
                    ir1e(li)=ir1em
                    r1dc(li)=r1dcm
...
750           continue
             ...
             nsubd1.gt.jsubmf+1.and.li.gt.lipl.and.l.gt.nbp) then
                iopbes=1
                end if
...
!  decide whether to use values of r1 and r1d for r2 and r2d
              ...
              if(iii.lt.0) iii=0
               ...
               -iii.lt.minacc).or.(knd.eq.kindd.and.match(li+1-ix)&
               -iii.lt.4)) then
...
!     1                             ir1de(li),r2c(li),ir2e(li),r2dc(li),
!     2                             ir2de(li),naccr
                    go to 1400
                    end if
                    if(ix.eq.0) then
                    ...
                    naccr=naccrt
                      ...
                      naccr=min(naccrpl,naccr1)
                      iopmatch=0
!                      if(naccr.gt.0) write(40,775) l,naccr
!775                   format(8x,'values for r2 and r2dc for l = ',i6,
!     1                       ' accurate to ',i2,' digits are given by'
!     2                       ' ir1 and ir1d')
                      end if
                    end if
                    if(ix.eq.1) then
...
!                      write(40,775) l,naccr
               ...
               match(li+1-ix)-iii.ge.minacc).or.(knd.eq.kindd&
               .and.match(li+1-ix)-iii.ge.4))) then
                    if(ix.eq.0) then
                    naccrt=min(match(li+1)-2,naccr1)
                    if(naccrt.lt.0) naccrt=0
                    naccr=naccrt
             ...
             .and.ndec-jsub.ge.naccr) iopleg=1
                if(x.lt.0.01e0_knd.and.iopleg.eq.0.and.l.ge.legstart&
             .and.ndec-jsub.ge.match(li+1-ix)-iii) iopleg=1
                if(x.ge.0.05e0_knd.and.x.le.0.9e0_knd.and.jsub.gt.&
             ndec-minacc.and.iopleg.eq.0.and.iopeta.eq.0) iopeta=4
             ...
             ndec-naccr) iopneu0=4
              ...
              iopmatch.eq.1.and.match(li+1-ix)-iii.lt.minacc&
                  .and.iopleg.eq.0) iopint=1
...
800           continue
              ...
              call r2int(l,m,cc,x,limint,ndec,nex,maxd,enr(li,:),dc01,&
                   idc01,maxint,maxmp,maxlp,intlim,rpint1,rpint2,&
...
805           continue
             ...
             .eq.naccrt.and.x.lt.0.1e0_knd)) then
                naccr=naccint
                jflagl=0
                r2c(li)=r2ic
                ir2e(li)=ir2ie
...
!     r2 calculation using a Legendre expansion and joining factor
             ...
             .or.(naccr.eq.naccrt.and.nmatch-2.ge.minacc.and.&
             x.ge.0.01e0_knd)) then
...
840             if(iqlegflag.eq.0) then
             ...
             real(cc).le.60.0e0_knd.and.real(cc).ge.10.0e0_knd&
             .and.m.le.40.and.x.le.0.99e0_knd.and.x.gt.0.1e0_knd)&
...
900           continue
              ...
              call r2leg(l,m,cc,x,lnum,minacc,limleg,limdr,iflagp,ndec,&
                   nex,maxd,maxmp,maxpdr,maxdr,maxq,enr(li,:),&
                   enrneg,drhor,nsdrhor1,nsdrho,dc01,idc01,dneg,&
                   idneg,nsdneg,dfnorm,idfe,dmfnorm,idmfe,prx,&
                   pdrx,qdrat,qdqr,qdm1,iqdm1,qdl,iqdl,qrat,qm1,&
                   iqm1,ql,iql,fajo,ifajo,ifsub,jsub,termpq,&
                   itermpq,ioppsum,iopqnsum,r1cin,ir1ein,r1dcin,&
                   ir1dein,naccr1,naccrpl,itestm,r2lc,ir2le,r2dlc,&
                   ir2dle,jleg,jlegp,jflagl,naccleg,kflagl,isub,&
                   isubd,nacccor)
              iopleg=2
...
910           continue
            ...
            .gt.minacc+1.and.li.gt.max(liplp+10,nbp)) then
                istartint=2
                iopint=0
                end if
             ...
             naccint.gt.nacclegp+2.and.istartleg.eq.1) iopleg=0
               ...
               ir1de(li)).gt.2.or.li.lt.nbp)) legstart=l+1
...
!920           if(knd.eq.kindd) write(40,820) naccleg,r2lc,ir2le,r2dlc,
!     1                                       ir2dle
!              if(knd.eq.kindq) write(40,825) naccleg,r2lc,ir2le,r2dlc,
!     1                                       ir2dle
              nacclegp=naccleg
...
!              if(knd.eq.kindd) write(40,820) naccleg1,r2l1c,ir2l1e,
!     1                                       r2dl1c,ir2dl1e
!              if(knd.eq.kindq) write(40,825) naccleg1,r2l1c,ir2l1e,
!     1                                       r2dl1c,ir2dl1e
960           continue
!
...
!     equal to 0
           ...
           .or.(naccr.ge.minacc.and.naccr.ne.naccrt.and.x.lt.&
...
1050          continue
            ...
            limneu0=jneu0max+jneu0max+40+&
              ...
              call r2neu0(l,m,cc,x,limneu0,ndec,nex,maxd,maxlp,maxn,&
                    minacc,enr(li,:),sneuf2,sneun2,ineue2,sneudf2,&
             ...
             li.gt.liplp+10.and.x.ge.0.1e0_knd) then
                 istartint=2
                 iopint=0
                 end if
!              if(knd.eq.kindd) write(40,820) naccneu0,r2nc,ir2ne,r2dnc,
!     1                                       ir2dne
!              if(knd.eq.kindq) write(40,825) naccneu0,r2nc,ir2ne,r2dnc,
!     1                                       ir2dne
...
!      r2 calculation using the variable eta expansion
           ...
           0.05e0_knd.and.naccr.lt.minacc.and.istarteta.ne.0)&
            iopeta=1
...
1210          continue
             ...
             limeta=jeta+jeta+40+&
              ...
              call r2eta(l,m,cc,x,etaval,nee,limeta,ndec,maxd,&
                   maxlp,maxn,maxp,minacc,wm,enr(li,:),sneufe,&
             ...
             naccleg.eq.naccrs.and.jflagl.eq.1))&
             then
                naccr=naccetasc
                naccetab=naccetasc
                jflagl=0
...
!1270          if(knd.eq.kindd) write(40,1280) naccetasc,etaval,nee,r2ec,
...
!     1               ' = ',f12.9,'; nee = ',i4,/,10x,'r2 = ', f19.15,
!     2               f19.15,i5,5x,'r2d = ',f19.15,f19.15,i5)
...
1320          continue
          ...
          naccrsav-5.and.x.gt.0.1e0_knd) then
                iopint=0
...
!     1               r1dc(li),ir1de(li),naccr1
!1340            format(1x,i5,2x,2(f17.14,f17.14,i6,2x),i2)
                go to 1400
                end if
                if(x.eq.0.0e0_knd) then
!                write(20,1350) l,r1c(li),ir1e(li),r1dc(li),ir1de(li),
!     1                   r2c(li),ir2e(li),r2dc(li),ir2de(li),naccr
!1350            format(1x,i5,2x,2(f17.14,f17.14,i6,2x),/,8x,
!     1                   2(f17.14,f17.14,i6,2x),i2,'e')
                go to 1400
                end if
...
!     1                r1dc(li-1),ir1de(li-1),r2c(li-1),ir2e(li-1),
!     2                r2dc(li-1),ir2de(li-1),naccrp
                  else
                  continue
...
!                  write(40,1395) naccr,l
              ...
              naccint.ne.naccr.and.jflagl.eq.1) jjflagl=1
                ...
                then
                  continue
!                  write(20,1350) l,r1c(li),ir1e(li),r1dc(li),ir1de(li),
!     1                         r2c(li),ir2e(li),r2dc(li),ir2de(li),
!     2                         naccr
                  else
                  continue
...
!     1                   r2c(li),ir2e(li),r2dc(li),ir2de(li),naccr
              ...
              go to 1400
!1380          format(1x,i5,2x,2(f17.14,f17.14,i6,2x),/,8x,
!     1                   2(f17.14,f17.14,i6,2x),i2,'w')
1390          continue
                if(ix.eq.1) then
...
!     1                     ir1de(li-1),r2c(li-1),ir2e(li-1),r2dc(li-1),
!     2                     ir2de(li-1),naccrp
                  else
                  r2c(li-1)=r1c(li)
                  ir2e(li-1)=ir1e(li)
...
1400          continue
             ...
             max(liplp,nbp).and.irtest.lt.-10) then
                iopneu0=0
                end if
              ir1ep=ir1e(li)
              naccrsavp=naccrsav
...
!  determine first kind oblate angular functions
              ...
              call s1leg(l,m,cc,iopang,iopnorm,barg,narg,lims1,ndec,nex,&
                   maxt,maxd,maxp,enr(li,:),dmlms,idmlmse,jsubms,&
                   pr,pdr,pdnorm,ipdnorm,pnorm,ipnorm,pdtempe,&
                   ipdtempe,pdtempo,ipdtempo,ptempe,iptempe,&
                   ptempo,iptempo,itestm,naccre,kindd,kindq,s1c,&
                   is1e,s1dc,is1de,naccs,naccds,jang,dmlms1,&
					 idmlms1e)
                do 1500 jarg=1,narg
                s1(li,jarg)=s1c(jarg)
...
!                if(iopang.eq.1) write(50,1420)
!     1                barg(jarg),naccs(jarg)
!                if(iopang.eq.2) write(50,1430)
!     1                barg(jarg),naccs(jarg),naccds(jarg)
!1420            format(1x,'eta = ',e24.15,'   accuracy = ',i2,
!     1                 ' digits.')
!1430            format(1x,'eta = ',e24.15,'   s1 and s1d accuracy = ',
!     1                 i2,' and ',i2,' digits.')
!                if(iopang.eq.1) write(30,1460)
!     1                barg(jarg),s1c(jarg),is1e(jarg),naccs(jarg)
!                if(iopang.eq.2) write(30,1470)
!     1                barg(jarg),s1c(jarg),is1e(jarg),s1dc(jarg),
!     2                is1de(jarg),naccs(jarg),naccds(jarg)
!                if(knd.eq.kindd.and.iopang.eq.1) write(50,1480)
!     1                s1c(jarg),is1e(jarg)
!                if(knd.eq.kindd.and.iopang.eq.2) write(50,1490)
!     1                s1c(jarg),is1e(jarg),s1dc(jarg),is1de(jarg)
!                if(knd.eq.kindq.and.iopang.eq.1) write(50,1485)
!     1                s1c(jarg),is1e(jarg)
!                if(knd.eq.kindq.and.iopang.eq.2) write(50,1495)
!     1                s1c(jarg),is1e(jarg),s1dc(jarg),is1de(jarg)
!1460            format(1x,f17.14,2x,f17.14,1x,f17.14,2x,i5,2x,', ',i2)
!1470            format(1x,f17.14,2x,f17.14,1x,f17.14,2x,i5,2x,f17.14,
!     1                 1x,f17.14,2x,i5,2x,i2,', ',i2)
!1480            format(12x,'s1 = ',f17.14,f17.14,2x,i5)
!1485            format(12x,'s1 = ',f35.31,f35.31,2x,i5)
!1490            format(12x,'s1 = ',f17.14,1x,f17.14,2x,i5,5x,'s1d = ',
!     1                 f17.14,1x,f17.14,2x,i5)
!1495            format(12x,'s1 = ',f35.31,1x,f35.31,2x,i5,/12x,'s1d = ',
!     1                 f35.31,1x,f35.31,2x,i5)
1500            continue
1510          continue
1515      continue
...
1540      continue
        continue
! DEALLOCATION
		deallocate(bliste,blisto)
		deallocate(gliste,glisto)
		deallocate(drhor)
		deallocate(pint1,pint2,pint3)
		deallocate(pint4,rpint1,rpint2)
		deallocate(sbesf,sbesdf,sbesfe)
		deallocate(sbesdfe,sbesfsv)
		deallocate(sbesdfsv)
		deallocate(ibese,ipnormint,ipnormint1)
		deallocate(ibesee)
		deallocate(ibesesv)
		deallocate(pnormint,pnormint1)
		deallocate(sbesdr,sbesn,sneudr1)
		deallocate(sneudr2,sneudre)
		deallocate(sneudrsv,sbesne)
		deallocate(sbesdre,sbesnsv)
		deallocate(sbesdrsv)
		deallocate(qdqr)
		deallocate(enrneg)
		deallocate(prat1)
		deallocate(ineue1,ineue2,ineuee)
		deallocate(ineuesv)
		deallocate(sneufe,sneudfe,sneufsv)
		deallocate(sneudfsv,sneuf1,sneudf1)
		deallocate(sneuf2,sneudf2,sneun1)
		deallocate(sneun2,sneune,sneunsv)
		deallocate(alpha,beta,coefa,coefb)
		deallocate(coefc,coefd,coefe,gamma)
		deallocate(pdr,pdrat,pdratt)
		deallocate(pr,prat,pratb)
		deallocate(pratbsv,prattsv)
		deallocate(pdrattsv,pratt)
		deallocate(pratb1,pratt1,pdratt1)
		deallocate(pratbsv1,prattsv1)
		deallocate(pdrattsv1)
		deallocate(prx,pdrx)
		deallocate(qrat,qdrat,qrat1,qdrat1)
		deallocate(arg,etainp,pdnorm)
		deallocate(pdnorma,pnorm,pnorma,pdtempe)
		deallocate(pdtempo,ptempe,ptempo,xin)
		deallocate(xlninp)
		deallocate(ipdnorm,ipdnorma,ipnorm)
		deallocate(ipnorma,ipdtempe,ipdtempo)
		deallocate(iptempe,iptempo,is1e,is1de)
		deallocate(naccs,naccds)
		deallocate(s1c,s1dc)
		deallocate(wr,xr)
		deallocate(eigt)
		deallocate(f,g,d,e)
		deallocate(eta,xbn,xln,wmeta2)
		deallocate(neeb,neeb1,limpsv)
		deallocate(limp1sv,limnsv,jelimsv)
		deallocate(jelim1sv,limjsv)
...
!  compute the angular function s1
          ...
          if(((abs(abs(barg(k))-1.0e0_knd)).lt.adec).and.(m.ne.0)) go to 220
          go to 230
...
!  complex(knd) scalars and arrays
        complex(knd) cc,coef,dmfnorm,dnew,dnewd,dold,doldd,r1c,r1ca,&
               r1d,r1dc,r1temp,r1tempa,r1dtemp,r1dtempa,&
               r1top,r1topa,r1dtop,term,termd
...
!     1         'available; forward series converged in ',i6,' terms.')
50	continue
!
...
100     iterm=0
 	...
 	r1dc=r1dc*ten**(-iterm)
        ...
        ir1de=ir1de-1
110	continue
!        if(nsubr+ndsubr.gt.0) write(40,120) nsubr,ndsubr
!120     format(15x,'sub. errors in the real parts of the num.',
!     1         ' of r1 and r1d are ',i2,' and ',i2,' digits.')
!        if(nsubi+ndsubi.gt.0) write(40,125) nsubi,ndsubi
...
!        if(naccns1r+naccns1i.gt.0) write(40,150) naccns1r,naccns1i
!150     format(15x,'subt. errors in the real and imag. parts of',
!     1         ' r1 numer. are ',i2,' and ',i2,' digits.')
!        if(naccns2r+naccns2i.gt.0) write(40,155) naccns2r,naccns2i
!155     format(15x,'subt. errors in the real and imag. parts of',
!     1         ' r1d numer. are ',i2,' and ',i2,' digits.')
!        if(ndensr+ndensi.gt.0) write(40,160) ndensr,ndensi
!160     format(15x,'subt. errors in the real and imag. parts of',
!     1         ' the denom. are ',i2,' and ',i2,' digits')
...
!  real(knd) scalars and arrays
        real(knd) aj,arr,c,coefa,coefn,coefme,coefmo,dec,dcon,ri,rm,&
             rm2,r2dposr,r2dposi,r2posr,r2posi,ten,teste,&
             testeo,x
...
!     1         ' available; ',i2,' and ',i2,' digits of sub. error'
!     2         ' in r2 and r2d')
        jterm=0
...
!  complex(knd) scalars and arrays
        complex(knd) cc,dfnorm,dmfnorm,dneg,dnegjf,dnew,dnewd,dold,&
               doldd,dc01,psum,pdsum,qndsum,qdsum,qdsump,qnsum,&
               qsum,qterm,r1c,r1dc,r2c,r2dc,spsum,spdsum,wronc,&
...
!     1                            jnleg,lim1,lim2,m,nsubleg,nsubdleg
...
!     1          ' the leading psum coefficient drhor(1).')
        return
        end
!
!
...
!  complex(knd) scalars
        complex(knd) arg,cc,eigval,ea,eb,e3,ra,rb,r1c,r1,r2,r2c,r2d,&
               r2dc,r3,sumi,sumdi,sumr,sumdr,termi,termdi,termr,&
               termdr
!  complex(knd) arays
...
!     1         ' terms; ',i6,' terms available.',/,15x,'r2 converged ',
!     2         'to ',i3,' digits with ',i3,' digits sub. error.',/,
!     3         15x,'r2d converged to ',i3,' digits with ',i3,' digits ',
!     4         'sub. error.')
...
!     1         i6,' ,',i6,' terms; ',i6,' terms available.',/,
!     2         15x,'best r2 at ',i6,' terms with convergence to ',i2,
!     3         ' digits; ',i2,' digits subtr. error.',/,15x,
!     4         'best r2d at ',i6,' terms with convergence to ',i2,
!     5         ' digits; ',i2,' digits subtr. error.',/,15x,
!     6         'estimated numerator and denominator accuracy is ',i2,
!     7         ' and ',i2,' digits.')
...
!        use param
        ...
        complex(knd) c,cp,eigst(np2),f(np),g(np),fm(np),gm(np),fp(np),&
               gp(np),fs(np),gs(np),p,testp,temp
!
        imax=limeig/2
...
!
        ...
        iflag=0
          ...
          limpsav=limp
           ...
           abs(real(f(i))).gt.0.001e0_knd) go to 20
            if(aimag(c).ne.0.0e0_knd.and.abs(aimag(f(i))-aimag(g(j)))/&
           abs(aimag(f(i))).gt.0.001e0_knd) go to 20
            limp=limp+1
            limpl=limpl+1
            temp=f(i)
              do k=i,limp+1,-1
...
!  locate prolate-like eigenvalues
        ...
        cp=(0.0e0_knd,-1.0e0_knd)*c
         ...
         aimag(f(i))).gt.0.02e0_knd) go to 60
            nf=nf+1
            fp(nf)=f(i)
            isav=i
              do k=i,imax-nf
...
70          do i=limpl+1,imax
         ...
         aimag(g(i))).gt.0.02e0_knd) go to 80
            ng=ng+1
            gp(ng)=g(i)
              do k=i,imax-ng
              g(k)=g(k+1)
...
!
        subroutine conver (l,m,lnum,cc,limd,blist,glist,blist1,glist1,&
                     ndec,maxd,ioprad,minacc,eigest,eigmat,lipl,&
                     lips,liplp,match,eigp,eign,kindd,kindq,&
...
140     continue
!        if(knd.eq.kindd.and.ioprad.ne.0.and.iflag.eq.0) write(40,150) l,
!     1           eigval,eigstart
!        if(knd.eq.kindd.and.ioprad.eq.0.and.iflag.eq.0) write(50,150) l,
!     1           eigval,eigstart
!        if(knd.eq.kindq.and.ioprad.ne.0.and.iflag.eq.0) write(40,155) l,
!     1           eigval,eigstart
!        if(knd.eq.kindq.and.ioprad.eq.0.and.iflag.eq.0) write(50,155) l,
!     1           eigval,eigstart
!150     format(1x,'l =',i5,6x,'eigenvalue =',e24.15,e24.15,/,16x,
!     1               ' estimate =',e24.15,e24.15)
!155     format(1x,'l =',i5,6x,'eigenvalue =',e39.31,e39.31,/,16x,
!     1               ' estimate =',e39.31,e39.31)
...
190     continue
        itestm=ndec
!        if(ioprad.ne.0) write(40,195) lim2,naccre
!        if(ioprad.eq.0) write(50,195) lim2,naccre
...
!        if(ioprad.ne.0) write(40,240) lim2,itestm,nlimp,naccre
!        if(ioprad.eq.0) write(50,240) lim2,itestm,nlimp,naccre
!240     format(7x,i7,' d coefs. Forward and backward recursion d ',
!     1         'ratios match to ',i2,' digits at n = ',i6,/,24x,
!     2         'estimated eigenvalue accuracy is ',i3,' digits')
          do 250 n=1,nlim
          enr(n)=enrf(n)
250       continue
260     continue
...
!                         are desired; set equal to 1 if angular
...
60      continue
        ...
        idmlmfe=-idmfe
!        if(ioprad.ne.0) write(40,70) jmf,jsubr,jsubi
!        if(ioprad.eq.0) write(50,70) jmf,jsubr,jsubi
!70      format(15x,'Morse-Feshbach norm. converged',
!     1         ' in ',i6,' terms with ',i2,' and ',i2,' digits of',
!     2         ' subtr.',/,23x,'error in the real and imag. parts')
        jsubmf=jsubr
...
110     continue
        ...
        idmlfe=-idfe
!        if(ioprad.ne.0) write(40,120) jf,jsubr,jsubi
!        if(ioprad.eq.0) write(50,120) jf,jsubr,jsubi
!120     format(15x,'Flammer norm. converged in ',
!     1         i6,' terms with ',i2,' and ',i2,' digits of ',
!     2         'subt.',/,22x,' error in real and imag. parts.')
        jsubf=jsubr
...
180     jsubms=int(log10((fterm/abs(dmsnorm))+dec))
        ...
        idmlmse=-idmse/2
!        if(iopang.ne.0) write(50,190) jn,jsubms
!190     format(5x,' Meixner-Schafke normalization converged in ',
!     1         i6,' terms with ',i2,' digits of subt. error')
200     continue
        return
        end
!
...
!   where coef = -1 when computing functions for r2leg, = +1 otherwise
!
!   for abs(eta) <= 0.1, calculate the ratios instead using a recursion
!   relation involving successive ratios (except for eta equal to 0,
!   where special expressions are used)
!
...
!  real(knd) scalars and arrays
        real(knd) ajm,dec,qdm1,qlow,qlow0,qlow1,qmid,qmid0,qmid1,qm1,&
            qmm,qupp,qupp0,qupp1,q00,q0m,q1m,q11,qmmm1,rin,rin1,&
...
!  real(knd) scalars and arrays
        real(knd) amo2,an,argb,arn,bn,coef,coefme,coefmo,coefo,dec,&
            ...
            teste,testeo,test1,test2,twom,twomi,x,x2,xsp1,zi,zl,&
            zu
...
150         continue
            ...
            p(2)=alpha(1)*etai*p(1)
             ...
             test1.or.abs(pint4(il+2)).gt.test1) then
                  limi=il
                  go to 160
                  end if
                end if
...
!     1         ', ',f12.10,', 'f12.10,'.',/,15x,'integrals for ',i5,
!     2         ' lowest order Legendre functions will be used for r2.')
!
